...
- 對 quota 敏感、長流程、或多階段 Gemini 工作（例如：identify → enrich → search → generate），優先先切成可獨立觀測與重試的**微步驟**，不要把整段工作包成單一大 call。
- 每個微步驟應有明確 action 名稱，並可被記錄、重試、或延後執行。
- 若同一個 job 內有多個細流程同時或連續使用 Gemini，**優先為每個 action 分配不同 preferred key**，避免同一把 key 在同一個 job 內被短時間重複消耗。
- preferred key 只是**軟偏好**：當健康 key 不足時，消費者可在 orchestration 層明確切換到共享輪替；但 `ai-core` 本身仍不得靜默降級，當池中無可用 key 時必須維持 `NoAvailableKeyError` 契約。
- 當 key 數量少於微步驟數量時，設計上應保證：
  1. 前幾個關鍵步驟優先拿到不同 key
  2. 後續步驟明確標記為 shared fallback
  3. 不可回報成「每步驟都已獨立分 key」
- 任何多流程 / 多 action key assignment 設計，都必須附帶驗證：
  - build / test 通過
  - 能證明不同 action 在 key 足夠時確實優先使用不同 key
  - 在 key 不足時行為可預測且不會卡死
